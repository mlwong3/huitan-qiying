...
降低長者手震與精準點擊門檻的自適應輸入系統：

- **防手震（One Euro Filter）** — Casiez 2012 自適應低通濾波：慢速強平滑壓手震、快速放開保持跟手。文具盤可一鍵開／關。
- **頭部操控** — 以 MediaPipe FaceLandmarker 用頭部移動控制游標；停留（dwell）1 秒即落筆／提筆，避開精準點擊，方便肢障使用者。
- **視線操控（實驗性）** — 以 WebGazer 用視線位置控制游標，同樣 dwell 落筆；與頭部操控互斥。
- **統一指標抽象層** — 觸控、頭部、視線（及未來語音）正規化成同一 `{x, y, action}` 訊號流，並共用同一 `DwellEngine`；繪畫核心與共繪同步皆無需改動。

> 🔒 **隱私**：頭部追蹤全部於瀏覽器**本機運算，影像不會上傳**。
