...
### Fixed
- **共繪重返恢復筆觸**：以前喺共繪房間用毛筆／禪繞／擦膠畫嘅自由筆觸，離開後重返會唔見晒
  （只有放置圖示同封存作品先會保留）。而家每一筆放低隻筆時都會同步入房間，重返或者有房友
  加入都會重繪返之前畫過嘅線條。
- **禪繞圖案間距**：修正禪繞筆圖案太密、擠成一條實線嘅問題，改為沿筆劃固定 26px 均勻分佈。
- **封存重複圖案**：修正獨畫模式封存作品後，重返該作品會出現兩個一樣圖案嘅問題。

### Technical
- 新增 socket `stroke` / `peer_stroke` 事件；房間狀態加 `room.strokes`，`init_room` 帶回歷史
  筆劃。client 端 `painter.emitStroke`（放筆時送出整筆）同 `painter.drawStroke`（重繪房友／歷史筆劃）。
- `drawZen` 改用 `ZEN_SPACING`（26px）常數＋跨線段累積距離 `zenAcc`。
- 移除 `finishDrawing` 獨畫分支多餘嘅 `board.addElement`。
- git commit `c1c9ff6`。

---

## [3.10.2] — 2026-07-15 · 「我的作品」新增刪除功能

### Added
- **刪除作品**：「我的作品」畫廊每張卡片右上角加咗刪除按鈕（紅色圓形 ×），一撳即刪，
  同步清除本機 localStorage 紀錄。

### Technical
- `app.deleteWork(index)` 移除指定作品並重繪；`renderMyWorks` 為每張卡片加刪除按鈕。
